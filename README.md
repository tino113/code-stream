...
```bash
# optional: choose host port (default is 8124)
export CODESTREAM_PORT=8124

docker compose up --build
```

Open:
- `http://localhost:8124/` (or the value of `CODESTREAM_PORT`)

### Why this fixes your error
The app now builds from a Dockerfile that copies `requirements.txt` into the image before startup. It no longer depends on bind-mount path assumptions at runtime, so `requirements.txt` is always present in the container.
